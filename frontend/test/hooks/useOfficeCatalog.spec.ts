// SPEC-016: useOfficeCatalog hook unit tests
...
describe("useOfficeCatalog", () => {
  ...
  it("does not fetch on mount when enabled is false", () => {
    mockGetOffices.mockResolvedValue([buildOffice()]);

    renderHook(() => useOfficeCatalog({ enabled: false }));
  ...
  it("fetches when enabled transitions to true", async () => {
    ...
    const { rerender, result } = renderHook(
      ({ enabled }: { enabled: boolean }) => useOfficeCatalog({ enabled }),
