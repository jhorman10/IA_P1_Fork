...
describe('RegisterAppointmentUseCaseImpl', () => {
    ...
    describe('execute', () => {
        ...
        it('should log registration event', async () => {
            ...
            expect(mockLogger.hasLog('Processing registration for patient 123456789')).toBe(true);
            expect(mockLogger.hasLog('Appointment registered for patient 123456789')).toBe(true);
        });
        ...
        it('should process multiple registrations independently', async () => {
            const command1 = new RegisterAppointmentCommand(
                111111111,
                'Patient One'
            );

            const command2 = new RegisterAppointmentCommand(
                222222222,
                'Patient Two'
            );
    ...
    describe('logging behavior', () => {
        it('should log all major steps', async () => {
            ...
            expect(logs.some(l => l.message.includes('Processing registration'))).toBe(true);
            expect(logs.some(l => l.message.includes('Appointment registered'))).toBe(true);
        });
        ...
        it('should include context in log entries', async () => {
            ...
            const logs = mockLogger.logs;
            expect(logs.some(l => l.context === 'RegisterAppointmentUseCase')).toBe(true);
        });
    });
});
