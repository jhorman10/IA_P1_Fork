...
import { Appointment as AppointmentSchemaClass, AppointmentDocument, AppointmentSchema } from '../../../../src/schemas/appointment.schema';
...
describe('MongooseAppointmentRepository (Integration)', () => {
    ...
    describe('findAvailableOffices', () => {
        it('should return all offices when none are occupied', async () => {
            ...
            const available = await repository.findAvailableOffices(['C1', 'C2', 'C3', 'C4']);
        ...
        it('should ignore completed appointments', async () => {
            ...
            const available = await repository.findAvailableOffices(['C1', 'C2', 'C3']);
    ...
    describe('findByIdCardAndActive', () => {
        ...
        it('should return null for non-existent IdCard', async () => {
            const found = await repository.findByIdCardAndActive(new IdCard(999999999));
    ...
    describe('updateStatus', () => {
        it('should update appointment status', async () => {
            ...
            await repository.save(apt);
            const mongoId = (await appointmentModel.findOne({ domainId: apt.id }))!._id.toString();
    ...
    describe('integration scenarios', () => {
        it('should handle complete appointment lifecycle', async () => {
            ...
            const waiting = await repository.findWaiting();
            expect(waiting).toContainEqual(expect.objectContaining({
                status: 'waiting'
            }));
            ...
            const availableAfterComplete = await repository.findAvailableOffices(['C1', 'C2', 'C3']);
        ...
        it('should handle multiple concurrent saves with same domainId', async () => {
            ...
            const final = await repository.findByIdCardAndActive(new IdCard(123456789));
