...
export class AssignAvailableOfficesUseCaseImpl implements AssignAvailableOfficesUseCase {
  ...
  async execute(): Promise<void> {
    this.logger.log(
      "--- INICIO ASIGNACIÓN DE CONSULTORIOS ---",
      "AssignAvailableOfficesUseCase",
    );
    const allOffices = Array.from({ length: this.totalOffices }, (_, i) =>
      String(i + 1),
    );
    this.logger.log(
      `Oficinas totales: ${allOffices.join(", ")}`,
      "AssignAvailableOfficesUseCase",
    );
    const freeOffices =
      await this.appointmentRepository.findAvailableOffices(allOffices);
    this.logger.log(
      `Oficinas libres detectadas: ${freeOffices.join(", ")}`,
      "AssignAvailableOfficesUseCase",
    );

    if (freeOffices.length === 0) {
      this.logger.log(
        "No hay oficinas libres para asignar.",
        "AssignAvailableOfficesUseCase",
      );
      return;
    }
    ...
    this.logger.log(
      `Turnos en espera detectados: ${waiting.map((a) => a.idCard.toValue()).join(", ")}`,
      "AssignAvailableOfficesUseCase",
    );
    if (waiting.length === 0) {
      this.logger.log(
    ...
    for (let i = 0; i < possibleAssignments; i++) {
      ...
      this.logger.log(
        `Asignando oficina ${office} al turno ${appointment.idCard.toValue()} (prioridad: ${appointment.priority.toValue()})`,
        "AssignAvailableOfficesUseCase",
      );
      const randomDuration = this.consultationPolicy.getRandomDurationSeconds();
      this.logger.log(
        `Duración asignada: ${randomDuration}s`,
        "AssignAvailableOfficesUseCase",
      );
      ...
      this.logger.log(
        `Assigned office ${office} to appointment ${appointment.idCard.toValue()}`,
        "AssignAvailableOfficesUseCase",
      );
    }
    this.logger.log(
      "--- FIN ASIGNACIÓN DE CONSULTORIOS ---",
      "AssignAvailableOfficesUseCase",
    );
  }
}
