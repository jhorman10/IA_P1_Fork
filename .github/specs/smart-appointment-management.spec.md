...
version: "1.0"
related-specs: [SPEC-004, SPEC-005]
---
...
Gestionar el registro, priorización y asignación automática de turnos a doctores disponibles. El sistema debe soportar colas priorizadas (high|medium|low) + FIFO, evitar duplicados por representante (idCard), exponer posición en cola y emitir eventos en tiempo real para pantallas públicas y dashboards internos.

### Requerimiento de Negocio

Como recepcionista quiero registrar turnos que se asignen al doctor más adecuado disponible de forma automática, de modo que la operación sea ágil y justa para pacientes y doctores.

### Historias de Usuario

#### HU-01: Registrar turno con prioridad y control de duplicado

```
Como:        Recepcionista o usuario público (vía formulario)
Quiero:      Registrar un turno con identificador de paciente (`idCard`) y prioridad
Para:        Ser ubicado en una cola priorizada evitando duplicados por `idCard`

Prioridad:   Alta
Estimación:  S
Dependencias: SPEC-004 (auth para endpoints protegidos), SPEC-005 (CI/tests)
...
```gherkin
CRITERIO-1.1: Registro exitoso de turno
  Dado que:  existe un formulario válido con `idCard` y `priority`
  Cuando:    se hace POST /appointments con datos válidos
  Entonces:  el sistema crea el turno, responde 201 con el id del turno
  Y:         publica evento `APPOINTMENT_CREATED` en RabbitMQ
```

**Error Path**

```gherkin
CRITERIO-1.2: Duplicado por idCard
  Dado que:  ya existe un turno activo para la misma `idCard`
  Cuando:    se intenta crear otro turno con la misma `idCard`
  Entonces:  el sistema responde HTTP 409 con mensaje "turno duplicado"
```

#### HU-02: Consultar posición en la cola

```
Como:        Paciente o recepcionista
Quiero:      Consultar la posición ordinal del turno en la cola
Para:        Estimar el tiempo de espera y actualizar la pantalla pública
...
```gherkin
CRITERIO-2.1: Obtener posición en cola
  Dado que:  existe un turno con id T-123 en status `waiting`
  Cuando:    GET /appointments/queue-position/:idCard
  Entonces:  responde 200 con `{ position: N }` donde N es 1-based
```

#### HU-03: Asignación automática a doctor disponible

```
Como:        Sistema de asignación
Quiero:      Asignar el siguiente turno al doctor más adecuado disponible
Para:        Minimizar tiempos de espera y balancear carga por especialidad

Prioridad:   Alta
Estimación:  M
Dependencias: SPEC-004 (perfiles/doctors), SPEC-008 (doctor landing — check-in status)
...
```gherkin
CRITERIO-3.1: Asignación por prioridad y disponibilidad
  Dado que:  existe doctor disponible con specialty requerida y cola con T-002(prio high)
  Cuando:    el ciclo de asignación se ejecuta (post-create o post-complete)
  Entonces:  T-002 se asigna al doctor y se publica `APPOINTMENT_ASSIGNED`
```

**Error Path / Edge**

```gherkin
CRITERIO-3.2: No hay doctor disponible
  Dado que:  cola no vacía pero no hay doctores disponibles
  Cuando:    se intenta asignar
  Entonces:  el turno queda en `waiting` y se reintenta periódicamente
```

### Reglas de Negocio

1. Ningún `idCard` puede tener más de un turno activo (`waiting` o `called`).
2. Orden: prioridad (high>medium>low) luego FIFO dentro de la misma prioridad.
3. Asignación respeta `specialty` si se provee; si no, hace asignación por disponibilidad general.
4. Producer publica eventos y responde antes de la transición definitiva que realiza el consumer.
...
#### `Appointment` (producer schema)

| Campo        | Tipo     | Obligatorio | Descripción                                          |
| ------------ | -------- | ----------- | ---------------------------------------------------- | --------------------------------- | --------- | ---------- |
| `id`         | ObjectId | sí          | Identificador único Mongo                            |
| `idCard`     | string   | sí          | Identificador del paciente (único por active turnos) |
| `priority`   | enum     | sí          | `high                                                | medium                            | low`      |
| `status`     | enum     | sí          | `waiting                                             | called                            | completed | cancelled` |
| `doctorId`   | string   | null        | no                                                   | Doctor asignado (cuando `called`) |
| `specialty`  | string   | null        | no                                                   | Especialidad requerida            |
| `created_at` | datetime | sí          | Timestamp UTC de creación                            |
| `updated_at` | datetime | sí          | Timestamp UTC de última modificación                 |

#### Índices / Constraints

- Índice compuesto: `{ priority: -1, created_at: 1 }` para selección eficiente por prioridad+FIFO.
- Índice único parcial sobre `idCard` para estados activos (`waiting`,`called`) para prevenir duplicados.

### API Endpoints

#### POST /api/v1/appointments

- **Descripción**: Registrar nuevo turno
- **Auth requerida**: opcional (public) / protegida si viene desde UI interna — validar por rol si aplica
- **Request Body**:
...
- **Response 201**: `{ "id": "<mongoId>", "status": "waiting" }`
- **Response 409**: duplicado por `idCard`

#### GET /api/v1/appointments/queue-position/:idCard

- **Descripción**: Devuelve posición ordinal
- **Response 200**: `{ "position": number }`

#### (Integración) Eventos publicados

- `APPOINTMENT_CREATED` — payload minimal: `{ id, idCard, priority, specialty, created_at }`
- `APPOINTMENT_ASSIGNED` — `{ id, doctorId, assignedAt }`
- `APPOINTMENT_UPDATED` — para cambios de status

### Arquitectura y Puertos

- Ports outbound esperados (producer):
  - `AppointmentRepositoryPort` — CRUD + findNextByPriority
  - `AppointmentLifecyclePublisherPort` — publicar eventos en RabbitMQ
  - `DoctorStatusPort` — consultar doctores disponibles (por specialty, office, estado)

- Consumer responsibilities:
  - Escuchar `APPOINTMENT_CREATED` y ejecutar motor de asignación
  - Escuchar `complete_appointment` / `cancel_appointment` y transicionar estados

### Diseño Frontend (alto nivel)

- Formulario público de creación de turno: `frontend/src/components/AppointmentForm`.
- Hook `useQueuePosition(idCard)` para consultar posición periódicamente.
- Pantalla pública `WaitingRoom` que consume eventos WebSocket para actualizaciones en tiempo real.

---

## 3. LISTA DE TAREAS (Checklist accionable)

### Backend

- [ ] Definir schema Mongoose `Appointment` y migraciones si aplica
- [ ] Implementar `AppointmentRepository` (findNextByPriority, insert, updateStatus)
- [ ] Implementar endpoint `POST /appointments` y `GET /appointments/queue-position/:idCard`
- [ ] Implementar publisher de eventos `APPOINTMENT_CREATED` en producer
- [ ] Implementar consumer: motor de asignación que escucha `APPOINTMENT_CREATED` y publica `APPOINTMENT_ASSIGNED`
- [ ] Tests unitarios para repository, service y motor de asignación
- [ ] Tests de integración (consumer + producer) con RabbitMQ mocked

### Frontend

- [ ] Crear `AppointmentForm` + validaciones
- [ ] Implementar `WaitingRoom` consumiendo WebSocket
- [ ] Hook `useQueuePosition`
- [ ] E2E: flujo crear turno → ver posición → recibir asignación en UI (auth-aware variant)

### QA

- [ ] Ejecutar `/gherkin-case-generator` para HU-01, HU-02, HU-03
- [ ] Ejecutar `/risk-identifier` para clasificar riesgos (ASD)

---

## 4. DEPENDENCIAS

- **Obligatorias**: SPEC-004 (auth & profiles) — para endpoints protegidos y vinculación `doctorId`.
- **Infra**: RabbitMQ (event bus), MongoDB (persistencia), WebSocket infra ya existente.
- **CI**: Requiere SPEC-005 ajustes de CI para ejecutar tests e2e auth-aware.

## 5. RIESGOS Y CONSIDERACIONES

- Concurrencia: la asignación debe diseñarse para evitar race conditions (use locking o findAndModify atómico en consumer).
- Scalabilidad: la estrategia de reintentos y backoff para asignación debe evitar ciclos intensivos bajo carga.
- Privacidad: la pantalla pública debe anonimizar datos — ver SPEC-009 para detalles de presentación.

## 6. CRONOGRAMA SUGERIDO Y ESTIMACIÓN

- Mínimo viable (MVP) estimado: 3-5 días hombre para Backend + 1-2 días Frontend + 2 días QA/tests (total 6-9 días)

---

**Autor:** spec-generator
**Estado inicial:** DRAFT — pedir aprobación técnica antes de iniciar implementación.
