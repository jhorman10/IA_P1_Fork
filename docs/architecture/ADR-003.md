...
### Current State
```typescript
// ❌ Anti-pattern: Monolithic validation
class AppointmentValidator {
  validate(apt: Appointment): ValidationResult {
    ...
    if (!apt.doctor.availability.includes(apt.date)) return fail("Doctor unavailable");
    // ... 5 more rules
    return succeed();
  }
}
...
export class DoctorAvailabilityPolicy implements ConsultationPolicy {
  ...
  evaluate(appointment: Appointment): PolicyResult {
    const isAvailable = appointment.doctor.availability
      .some(slot => slot.contains(appointment.date));
...
export class ConsultingRoomPolicy implements ConsultationPolicy {
  ...
  evaluate(appointment: Appointment): PolicyResult {
    ...
    const isAvailable = appointment.room
      .getAvailability(appointment.date)
      .isAvailable;
...
export class CreateAppointmentUseCase {
  ...
  async execute(
    ...
    for (const policy of policies) {
      ...
      if (result.isErr()) {
        return Result.err(
          `Policy "${policy.name}" failed: ${result.error}`
        );
      }
    }
