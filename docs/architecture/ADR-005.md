...
### Current Challenge
```typescript
// ❌ Tight coupling without Domain Events
Producer Service:
...
export class CreateAppointmentUseCase {
  ...
  async execute(
    ...
    return Result.ok(
      new CreateAppointmentResponse(appointment.id.value)
    );
  }
}
```
...
- **Evans, E.** (2003). *Domain-Driven Design: Tackling Complexity*. Addison-Wesley.
